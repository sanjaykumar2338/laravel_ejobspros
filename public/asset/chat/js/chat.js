$(document).ready(function(){
	...
	var main_url = $('#main_url').val()+'/dashboard/chat_action';
	...
	$(document).on('focus', '.message-input', function(){
		var is_type = 'yes';
		$.ajax({
			url: $('#main_url').val()+'/dashboard/chat_action',
			method:"POST",
			data:{is_type:is_type, action:'update_typing_status',"_token": $('#token').val()},
			success:function(){
			}
		});
	}); 
...
function updateUserList() {
	$.ajax({
		...
		method:"POST",
		dataType: "json",
		data:{action:'update_user_list',"_token": $('#token').val()},
...
function sendMessage(to_user_id) {
	...
	$.ajax({
		url: $('#main_url').val()+'/dashboard/chat_action',
		method:"POST",
		data:{to_user_id:to_user_id, chat_message:message, action:'insert_chat',"_token": $('#token').val()},
		dataType: "json",
...
function showUserChat(to_user_id){
	$.ajax({
		url: $('#main_url').val()+'/dashboard/chat_action',
		method:"POST",
		data:{to_user_id:to_user_id, action:'show_chat',"_token": $('#token').val()},
		dataType: "json",
		success:function(response){
...
function updateUserChat() {
	$('li.contact.active').each(function(){
		...
		$.ajax({
			url: $('#main_url').val()+'/dashboard/chat_action',
			method:"POST",
			data:{to_user_id:to_user_id, action:'update_user_chat',"_token": $('#token').val()},
			dataType: "json",
			success:function(response){				
				$('#conversation').html(response.conversation);			
			}
	...
	$('li.contact').each(function(){
		if(!$(this).hasClass('active')) {
			...
			$.ajax({
				url: $('#main_url').val()+'/dashboard/chat_action',
				method:"POST",
				data:{to_user_id:to_user_id, action:'update_unread_message',"_token": $('#token').val()},
				dataType: "json",
				success:function(response){		
					if(response.count) {
...
function showTypingStatus() {
	$('li.contact.active').each(function(){
		...
		$.ajax({
			url: $('#main_url').val()+'/dashboard/chat_action',
			method:"POST",
			data:{to_user_id:to_user_id, action:'show_typing_status',"_token": $('#token').val()},
			dataType: "json",
			success:function(response){				
